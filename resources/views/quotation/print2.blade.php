...
<head>
  ...
  <style>
    ...
    .invoice-items-table th, .invoice-items-table td { border:1px solid #dee2e6; padding:6px 6px; }
    ...
    .signature-boxes { display:flex; justify-content:space-between; gap:18px; align-items:flex-end; flex-wrap:nowrap; }
    .signature-box { flex:1; text-align:center; }
    .signature-line { width:60%; height:28px; border-bottom:1px solid #000; margin:0 auto 6px; }
    ...
    @media print {
      ...
      .signature-boxes { display:flex !important; flex-direction:row !important; }
    }
  </style>
</head>
<body>
  @php 
    ...
    $logoUrl = null;
    if($business && !empty($business->businessLogo)){
      $logoUrl = (strpos($business->businessLogo, 'http') === 0)
        ? $business->businessLogo
        : asset('public/uploads/business/' . ltrim($business->businessLogo, '/'));
    }
    $currencyName = null;
  ...
  <div id="rn-invoice-root">
    <div class="card-body">
      ...
      @if(!empty($quote->notes))
        <div style="margin-top:10px;"><strong>Notes:</strong><div>{{ $quote->notes }}</div></div>
      @endif
  ...
  <script>
    try{ window.onload = function(){ window.print(); setTimeout(function(){ window.close(); }, 500); } }catch(e){}
  </script>
</body>
</html>
